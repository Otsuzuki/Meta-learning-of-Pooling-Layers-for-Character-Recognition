...
import layers
from synthetic_loader import MiniImagenetDataset, OmniglotDataset
    ...
    for i in range(task_num):
        with higher.innerloop_ctx(
            ...
            for _ in range(n_inner_iter):
                input_list = [step_idx, x_spt[i].to(device), 0, "train"]
                spt_pred, w = fnet(input_list)
                spt_loss = criterion(spt_pred.float(), y_spt[i].long())
                diffopt.step(spt_loss)
            input_list = [step_idx, x_qry[i].to(device), 0, "train"]
...
def test(device, step_idx, data, net, inner_opt_builder, n_inner_iter):
    ...
    for i in range(task_num):
        with higher.innerloop_ctx(
            ...
            total = 0
            for _ in range(n_inner_iter):
                input_list = [step_idx, x_spt[i].to(device), 0, "val"]
                spt_pred, w = fnet(input_list)
                spt_loss = criterion(spt_pred.float(), y_spt[i].long())
                diffopt.step(spt_loss)
            input_list = [step_idx, x_qry[i].to(device), 0, "val"]
            qry_pred, w = fnet(input_list)
            qry_loss = criterion(qry_pred.float(), y_qry[i].long())
            qry_losses.append(qry_loss.detach().cpu().numpy())
...
def main():
    ...
    parser.add_argument('--hidden-size', type=int, default=64, help='Number of channels in each convolution layer of the VGG network ')
    ...
    for step_idx in range(args.num_outer_steps):
        ...
        if step_idx < 60000:
            ...
            if step_idx == 0 or (step_idx + 1) % 20 == 0:
                ...
                with open("./w/p.csv", 'a') as f:
                    np.savetxt(f, net.first_layer[0].p_norm.detach().cpu().numpy(), delimiter=',')
    ...
    torch.save(model.state_dict(), './w/model' + str(num) + '.pth')
