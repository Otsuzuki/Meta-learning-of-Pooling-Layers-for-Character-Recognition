...
class ShareConv2d(nn.Module):
    def __init__(self, mask, imgsz, kernel_size, stride, p, bias=True):
        super(ShareConv2d, self).__init__()
        self.mask = Parameter(mask, requires_grad=True)
        # self.p = Parameter(torch.zeros(p).add_(20), requires_grad=True)
        ...
        self.fold = nn.Fold(output_size=(14, 14), kernel_size=(1, 1))
        if bias:
    ...
    def pooling(self, x):
        ...
        # z = self.unfold(z)
        # z = z.reshape(x[1].shape[0],x[1].shape[1],9,z.shape[2])
        # z = torch.abs(z)
...
class MetaLinearModel(nn.Module):
    def __init__(self):
        ...
        self.first_layer = nn.Sequential(layers.ShareConv2d(self.mask, args.imgsz, args.kernel_size, args.stride, args.p, bias=False)).to(device)
        self.fc4_layers = nn.Sequential(
                                nn.Linear(1 * 14 * 14, 128, bias=False),
...
class MetaConvModel(nn.Module):
    def __init__(self, args):
        ...
        self.mask = torch.rand(args.kernel_size ** 2, args.p)
        self.first_layer = nn.Sequential(ShareConv2d(self.mask, args.imgsz, args.kernel_size, args.stride, args.p, bias=False))
        self.second_layer = nn.Sequential(
                                nn.Linear(64 * 14 * 14, 512, bias=False),
                                nn.ReLU(inplace=True),
                                nn.Linear(512, 5, bias=False),
                                nn.ReLU(inplace=True),
                            )
        self.features = nn.Sequential(OrderedDict([
            ('layer1', conv_block(in_channels=1, hidden_size=64)),]))
