...
class STEP(Function):
    ...
    def forward(ctx, input):
        ...
        return (torch.sign(input-0.5) + 1) / 2
...
class ShareConv2d(nn.Module):
    def __init__(self, args, mask, imgsz, kernel_size, stride, p, bias=False):
        super(ShareConv2d, self).__init__()
        self.args = args.kernel_size
        self.mask = Parameter(mask, requires_grad=False)
        self.p_norm = Parameter(torch.zeros(p).add_(4), requires_grad=False)
