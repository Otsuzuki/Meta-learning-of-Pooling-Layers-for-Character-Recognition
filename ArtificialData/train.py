...
import higher
import layers
from layers import lp_pooling1d, lp_pooling2d
from loader import Loader
from inner_optimizers import InnerOptBuilder
from figure import figure_loss, Visualize_W1d, Visualize_W2d, Visualize_W2d_color
...
def main():
    ...
    parser.add_argument("--problem", type=str, default="Square_2d")
    parser.add_argument("--model", type=str, default="Pooling_2d")
    parser.add_argument("--device", type=str, default="cpu")
    parser.add_argument('--input_size', type=int, help='input_size', default=28)
    parser.add_argument('--output_size', type=int, help='output_size', default=196)
    ...
    db = Loader(device, problem=args.problem, k_spt=args.k_spt, k_qry=args.k_qry)

    if args.model == "Pooling_1d":
        mask = torch.rand(args.output_size, args.input_size)
        net = nn.Sequential(layers.lp_pooling1d(mask, args, bias=False)).to(device)
    elif args.model == "Pooling_2d":
        mask = torch.rand(args.kernel_size ** 2, args.output_size)
        net = nn.Sequential(layers.lp_pooling2d(device, mask, args.input_size, args.kernel_size, args.stride, args.output_size, bias=False)).to(device)
    else:
    ...
    for step_idx in range(args.num_outer_steps):
        ...
        if step_idx < 60000:
            ...
            if step_idx == 0 or (step_idx + 1) % 20 == 0:
                if args.model == "Pooling_1d":
                    Visualize_W1d(net, step_idx, w)
                elif args.model == "Pooling_2d":
                    Visualize_W2d(net, step_idx, w, args)
                    Visualize_W2d_color(net, step_idx, args)
